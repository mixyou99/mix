...
> **강사 노트(학생 배포본에는 빼기):** 상위 단어는 실측(tableA1 클래식·tableA2 스마트). 심어진 **과잉 해석(over-read)**:
> **① Smart #3(1차 함정)** — 실제는 *이어폰/헤드폰* 토픽인데, 단어 "quality"가 보인다는 이유로 AI가 **"제품 품질
> 심층 리뷰"** 로 명명하고 고품질·관련 콘텐츠로 취급. **② Smart #10** — 센서+UI+일반어가 섞인 다주제인데 **"스마트
> 피트니스 스위트"** 로 단정. **③ Classic #8** — 상위 단어의 절반이 잡담(one·little·bit·good·other)인데 **"시계
> 기능"** 으로 과잉. **대조(AI가 맞게 읽음): Classic #4** — 명백한 보석/주얼리라 AI도 off-topic으로 바르게 판정
> (AI가 항상 틀리는 게 아님을 보여, #3의 실패를 더 선명하게). **코더 관련성(정답 키)은 여기 없다 — 워크시트에서
> 공개**해 학생이 먼저 단어로 잡고, 그다음 확인하게 한다.*
