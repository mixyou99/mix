...
REASONS = [
    ...
    ("날짜 구성요소", _ctx(r"2021-")),
    # 교원 노트의 세션 운영 분 배분 (2026-09-10 추가). 원고 값이 아니라 우리 문서의
    # 운영 수치다 — `별지 문항 번호` 와 같은 부류다. 문맥 창(±45자)이 넓으므로
    # 표기를 `(10분)` · `(10 min)` 괄호형으로 못 박아 좁힌다. 맨 `\d+분` 으로 열면
    # "표 3 분석" 같은 데서 오탐이 난다.
    # 닫는 괄호를 요구하지 않는다 — `(75분 + 75분)` 같은 합산 표기가 걸리지 않았다.
    # 여는 괄호 직후의 숫자만 보므로 "표 3 분석" 류 오탐은 여전히 나지 않는다.
    ("세션 운영 분 배분", _ctx(r"\(\d+\s*분|\(\d+\s*min")),
    # 교원 노트가 별지 문항의 **항 번호**를 가리킨다 (2026-09-10 추가).
    # `F-\d` 와 같은 부류 — 원고 값이 아니라 우리 문서의 구조 번호다.
    # KR `3항` · EN `Part 3`. 숫자가 바로 앞에 붙을 때만 본다 — "다섯째 항목" 은 안 걸린다.
    ("별지 문항 항 번호", _ctx(r"\d\s*항|[Pp]arts? \d")),
    # 교원 노트의 블록 번호 (2026-09-10 추가). ⚠ 위 사각지대가 **실제로 발현한** 자리다 —
    # `1블록` · `2블록` 의 1·2 가 REASONS 에 걸리지 않아 표 1 최대값·누적리뷰수 최소값으로
    # 오귀속됐다(게이트는 통과했으나 사유가 거짓이었다). 한 자리 토큰이라 strong 이 아니어서
    # REASONS 가 화이트리스트보다 먼저 돈다 — 그래서 여기 등록하면 순서를 바꾸지 않고 잡힌다.
    ("교원 노트 블록 번호", _ctx(r"\d\s*블록|[Bb]lock \d")),
]

# ⚠ 잔존 사각지대 (2026-09-10): g_token_census 는 두 자리 이상 토큰을 **REASONS 보다
#    먼저** 화이트리스트에 대조한다. 그래서 운영 수치가 팩트표 값과 우연히 같으면
#    (예: 분 배분에 40 을 쓰면 "관측 기간 40주" 로) **틀린 사유가 붙은 채 통과**한다.
#    이번 교원 노트가 쓰는 값(60·75·5·7·8·10·12·13·15·20·25·30·35·150)은 전건
#    화이트리스트와 겹치지 않음을 실측으로 확인하고 채택했다. 순서 자체는 바꾸지
#    않았다 — 바꾸면 팩트표 적중 셈이 전부 흔들린다. 새 운영 수치를 쓸 때마다
#    화이트리스트와의 충돌을 먼저 확인할 것.
